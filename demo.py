    ...
    for c_idx, sentences in enumerate(cover_texts):
        for s_idx, sen in enumerate(sentences):
            ...
            if len(valid_watermarks) > 1:
                ...
                num_digit = math.floor(math.log2(len(valid_watermarks)))
                if len(message):
                    remaining_message = message[:num_digit]
                else:
                    dummy_message = "0" * 20
                    remaining_message = message[:num_digit]
                message = message[num_digit:]

                random_msg_decimal = int(remaining_message, 2)
